describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
     ...
     */
    const productItem = await page.$('product-item'); // get first product-item
    const shadowRoot = await productItem.getProperty('shadowRoot'); // access shadow DOM
    const button = await shadowRoot.$('button'); // find button inside shadow DOM

    await button.click(); // simulate click

    const innerTextProp = await button.getProperty('innerText'); // get innerText
    const innerText = await innerTextProp.jsonValue(); // extract string
 ...
 await page.$$eval('product-item', items => {
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
     ...
     */
    await page.reload();
    // Wait briefly for localStorage to be updated
    await new Promise(resolve => setTimeout(resolve, 100));
    const cart = await page.evaluate(() => localStorage.getItem('cart'));
    console.log('Cart after adding all items:', cart);
